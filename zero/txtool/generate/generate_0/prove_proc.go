...
type gen_input_desc struct {
	desc  c_czero.InputDesc
	index int
	e     error
}

func (self *gen_input_desc) Run() error {
	tk := c_czero.Sk2Tk(&self.desc.Sk)
	if !c_czero.IsMyPKr(&tk, &self.desc.Pkr) {
...
type gen_output_desc struct {
	desc  c_czero.OutputDesc
	index int
	e     error
}

func (self *gen_output_desc) Run() error {
	if err := c_czero.GenOutputProof(&self.desc); err != nil {
		self.e = err
		return err
	} else {
		return nil
...
func (self *gen_ctx) genDesc_Zs() (e error) {
	...
	if self.param.Cmds.PkgCreate != nil {
		...
		g.desc.Key = pkg.GetKey(&self.param.From.PKr, c_czero.Sk2Tk(self.param.From.SKr.ToUint512().NewRef()).NewRef())
